...
import type { DisplayTarget } from '../shared/window-types'
...
/**
 * 注入的 screen 模块形状，仅暴露本模块需要的方法。
 */
export interface ScreenLike {
  getAllDisplays(): DisplayInfo[]
  getPrimaryDisplay?(): DisplayInfo
  getDisplayMatching?(bounds: DisplayBounds): DisplayInfo
...
/**
 * 用于在 centerToParent 中读取父窗口边界的最小接口。
 */
 ...
 * @returns 显示器信息数组。
 */
export function getAllDisplays(screen: ScreenLike): DisplayInfo[] {
 ...
 * @returns 主显示器信息。
 */
export function getPrimaryDisplay(screen: ScreenLike): DisplayInfo {
...
export function selectTargetDisplay(
  screen: ScreenLike,
  strategy: DisplayTarget,
  options: {
    parent?: ParentWindowLike | null
 ...
 * @returns 是否可见。
 */
export function isBoundsVisible(displays: DisplayInfo[], bounds: WindowBounds): boolean {
 ...
 * @returns 居中后的边界。
 */
export function centerToDisplay(
  display: DisplayInfo,
  windowWidth: number,
  windowHeight: number
): WindowBounds {
 ...
 * @returns 居中后的边界。
 */
export function centerToParentWindow(
 ...
 * @returns 校正后的边界。
 */
export function pullBackToVisible(
  displays: DisplayInfo[],
  bounds: WindowBounds
): WindowBounds {
 ...
 * @returns 校正后的边界。
 */
export function autoCorrectBounds(
  screen: ScreenLike,
  bounds: WindowBounds,
  minWidth: number,
  minHeight: number,
