...
exports.filterRoutesByPermission = filterRoutesByPermission;
exports.filterMenuByPermission = filterMenuByPermission;
/**
 * 判断是否拥有指定权限。
 *
 ...
 * @returns 过滤后的路由列表。
 */
function filterRoutesByPermission(routeList, permissions) {
    return routeList.filter((route) => hasAllPermissions(permissions, route.meta.permissions));
}
/**
 * 过滤菜单（与 utils/menu.ts 配合使用）。
 */
function filterMenuByPermission(items, permissions, checkFn) {
    const result = [];
    for (const item of items) {
        if (!checkFn(item, permissions))
            continue;
        if (item.children) {
